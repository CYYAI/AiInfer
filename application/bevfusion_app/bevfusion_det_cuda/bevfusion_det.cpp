#include "bevfusion_det.hpp"
namespace tensorrt_infer
{
    namespace bevfusion_cuda
    {
        void BevFusionDetect::initParameters(const std::string &engine_file, float score_thr, float nms_thr)
        {
        ...
        BevFusionDetect::~BevFusionDetect()
        {
            CHECK(cudaStreamDestroy(cu_stream)); // 销毁cuda流
        }

        void BevFusionDetect::adjust_memory(int batch_size)
        {
            // 申请模型输入和模型输出所用到的内存
        ...
        void BevFusionDetect::preprocess_gpu(int ibatch, const Image &image,
        ...
        void BevFusionDetect::postprocess_gpu(int ibatch, cudaStream_t stream_)
        {
        ...
        BatchBoxArray BevFusionDetect::parser_box(int num_image)
        {
        ...
        BoxArray BevFusionDetect::forward(const Image &image)
        {
        ...
        BatchBoxArray BevFusionDetect::forwards(const std::vector<Image> &images)
        {
