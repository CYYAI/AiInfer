#include "yolo_series_detect.hpp"
namespace tensorrt_infer
{
    namespace yolo_series_cuda
    {
        void YOLOSeriesDetect::initParameters(const std::string &model_name, const std::string &engine_file, float score_thr, float nms_thr)
        {
            ...
            this->model_type = name_type(model_name);
            ...
            // 对输入的图片预处理进行配置,即，yolov5,v6,v7的预处理是除以255，并且是RGB通道输入
            if (this->model_type == Type::V5 || this->model_type == Type::V6 || this->model_type == Type::V7)
                model_info->m_preProcCfg.normalize_ = Norm::alpha_beta(1 / 255.0f, 0.0f, ChannelType::RGB);
            else if (this->model_type == Type::X)
                model_info->m_preProcCfg.normalize_ = Norm::None();
            ...
            if (model_info->m_postProcCfg.num_classes_ == 0)
                model_info->m_postProcCfg.num_classes_ = model_info->m_postProcCfg.bbox_head_dims_[2] - 5;
        ...
        YOLOSeriesDetect::~YOLOSeriesDetect()
        {
            CHECK(cudaStreamDestroy(cu_stream)); // 销毁cuda流
        }

        void YOLOSeriesDetect::adjust_memory(int batch_size)
        {
            // 申请模型输入和模型输出所用到的内存
        ...
        void YOLOSeriesDetect::preprocess_gpu(int ibatch, const Image &image,
        ...
        void YOLOSeriesDetect::postprocess_gpu(int ibatch, cudaStream_t stream_)
        {
            ...
            decode_detect_kernel_invoker(image_based_bbox_output, model_info->m_postProcCfg.bbox_head_dims_[1], model_info->m_postProcCfg.num_classes_,
        ...
        BatchBoxArray YOLOSeriesDetect::parser_box(int num_image)
        {
            ...
            for (int ib = 0; ib < num_image; ++ib)
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    // if (keepflag == 1)
                    // {
                    //     std::cout << pbox[0] << ", " << pbox[1] << ", " << pbox[2] << ", " << pbox[3] << ", " << pbox[4] << ", " << label << std::endl;
                    //     output.emplace_back(pbox[0], pbox[1], pbox[2], pbox[3], pbox[4], label);
                    // }
                }
            }
        ...
        BoxArray YOLOSeriesDetect::forward(const Image &image)
        {
        ...
        BatchBoxArray YOLOSeriesDetect::forwards(const std::vector<Image> &images)
        {
