#include "yolov8_face_detect.hpp"
namespace tensorrt_infer
{
    namespace yolov8_cuda
    {
        void YOLOv8Detect::initParameters(const std::string &engine_file, float score_thr, float nms_thr)
        {
            ...
            // 加载infer对象
            this->trt_ = trt::infer::load(engine_file);
            ...
            model_info->m_preProcCfg.isdynamic_model_ = this->trt_->has_dynamic_dim();
        ...
        void YOLOv8Detect::adjust_memory(int batch_size)
        {
            ...
            // 申请模型解析成box时需要存储的内存,32是为了防止内存溢出，多增加的
        ...
        void YOLOv8Detect::postprocess_gpu(int ibatch, cudaStream_t stream_)
        {
            ...
            decode_detect_yolov8_kernel_invoker(image_based_bbox_output, model_info->m_postProcCfg.bbox_head_dims_[2], model_info->m_postProcCfg.num_classes_,
                                                model_info->m_postProcCfg.bbox_head_dims_[1], model_info->m_postProcCfg.confidence_threshold_,
                                                affine_matrix_device, boxarray_device, model_info->m_postProcCfg.MAX_IMAGE_BOXES, stream_);

            // 对筛选后的框进行nms操作
            nms_kernel_invoker(boxarray_device, model_info->m_postProcCfg.nms_threshold_, model_info->m_postProcCfg.MAX_IMAGE_BOXES, stream_);
        }
        ...
        void YOLOv8Detect::draw_batch_rectangle(std::vector<cv::Mat> &images, BatchBoxArray &batched_result, const std::string &save_dir)
        {
        ...
        void YOLOv8Detect::draw_one_image_rectangle(cv::Mat &image, BoxArray &result, const std::string &save_dir)
        {
        ...
        BatchBoxArray YOLOv8Detect::forwards(const std::vector<Image> &images)
        {
            ...
            if (!trt_->forward(bindings, cu_stream))
            {
