...
void det_track_infer(ai::arg_parsing::Settings *s)
{
    // yolov8 det初始化
    ...
    while (true)
    {
        ...
        ai::cvUtil::BoxArray res_boxes = yolov8_obj.forward(ai::cvUtil::cvimg_trans_func(img));
        timer.start();
...
int main(int argc, char *argv[])
{
    ...
    det_track_infer(&s);               // tensorrt的gpu版本推理：模型前处理和后处理都是使用cuda实现
