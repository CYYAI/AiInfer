...
class SubclassHeadBBox(nn.Module):
    ...
    def head_forward(self, inputs, classes_eye):
        ...
        lidar_feat_flatten = lidar_feat.view(batch_size,
                                             ...
                                             -1)  # [BS, C, H*W]
        bev_pos = self.bev_pos.to(lidar_feat.dtype).repeat(
        ...
        #     ..., : self.num_proposals
        # ]
        top_proposals = heatmap.view(batch_size, -1).topk(k=self.num_proposals,
                                                          dim=-1,
                                                          largest=True)[1]
    ...
    def get_bboxes(self, preds_dict, one_hot):
        ...
        return [
            batch_score, batch_rot, batch_dim, batch_center, batch_height,
            batch_vel
        ]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Export transfusion to onnx file")

    parser.add_argument(
        '--ckpt',
        type=str,
        default='bevfusion_qat/quant_model/ckpt/bevfusion_ptq.pth')
    parser.add_argument('--quant_precision',
                        type=str,
                        default='int8',
                        choices=['int8', 'fp16'])
...
if __name__ == "__main__":
    ...
    suffix = args.quant_precision
    if suffix == "fp16":
        quantize.disable_quantization(model).apply()

    save_root = os.path.join(args.save_dir, f"onnx_{suffix}")
    ...
    with torch.no_grad():
        ...
        fuser_onnx_path = os.path.join(save_root, "fuser.onnx")
        torch.onnx.export(
            fuser,
        ...
        print(
            f"🚀 The export is completed. ONNX save as {fuser_onnx_path} 🤗, Have a nice day~"
        )

        boxhead_onnx_save_path = os.path.join(save_root, "head.bbox.onnx")
        head_input = torch.randn(1, 512, 180, 180).cuda()
        torch.onnx.export(
            headbbox,
            head_input,
            boxhead_onnx_save_path,
            opset_version=13,
            input_names=["middle"],
            output_names=["score", "rot", "dim", "reg", "height", "vel"],
        )
        print(
            f"🚀 The export is completed. ONNX save as {boxhead_onnx_save_path} 🤗, Have a nice day~"
        )
