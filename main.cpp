...
#include "application/trt_yolov8_cuda/yolov8_face_detect.hpp"
#define DEVICE_NUM 1
...
void trt_cuda_inference(ai::arg_parsing::Settings *s)
{
    ...
    for (int i = 0; i < s->loop_count; ++i)
    {
        ...
        timer.stop("one image infer time:");
    }
    timer.stop(ai::utils::path_join("Batch=%d,loop iters=%d,run infer mean time:", s->batch_size, s->loop_count).c_str(), s->loop_count);
